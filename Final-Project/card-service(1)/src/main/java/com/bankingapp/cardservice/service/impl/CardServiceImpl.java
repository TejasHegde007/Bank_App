...
public class CardServiceImpl implements CardService {
    ...
    @Override
    public Card chargeCard(Long id, BigDecimal amount) {
        Card card = getCardById(id);

        if (card.getCardType() != CardType.CREDIT) {
            throw new CardException("Charges are only allowed on credit cards");
        }
        if (card.getIsBlocked()) {
            throw new CardException("Card is blocked");
        }
        if (amount.compareTo(BigDecimal.ZERO) <= 0) {
            throw new CardException("Amount must be positive");
        }
        if (card.getAvailableLimit().compareTo(amount) < 0) {
            throw new CardException("Insufficient available limit");
        }
